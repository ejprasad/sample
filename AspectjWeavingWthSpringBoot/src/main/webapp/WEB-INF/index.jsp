...
function fire_ajax_submit() {
    ...
    reqBody["board"] = $("#board").val();
    reqBody["aadhaar"] = $("#aadhaar").val();
    ...
    alert("nochance")
	    ...
	    $.ajax({
	        ...
	        headers: {
	            'WWW-Authenticate':digest_header + ', username="uname"',
	            //'X-XSRF-TOKEN':xsrf,
	            //'Content-Type':'application/json'
	            //Digest = tokens[0]
	            //realm:tokens[1].substr(tokens[1].indexOf("=") + 1), 
	            //qop:tokens[2].substr(tokens[2].indexOf("=") + 1),
	            //nonce:tokens[3].substr(tokens[3].indexOf("=") + 1)
	        },
    	...
    	$.ajax({
            ...
            headers: {
                'Authorization':'Digest username="uname"',
            ...
            error: function (request, textStatus, e) {
	            ...
	            quotedString = unq(quotedString)

	            tokens = request.getResponseHeader('WWW-Authenticate').match(tokenizer);
	            tokens[1].replace(/"/g,);//realm
	            tokens[2].replace(/"/g,);//qop
	            tokens[3].replace(/"/g,);//nonce
	            digest_header = request.getResponseHeader('WWW-Authenticate');
	            fire_ajax_submit();
            }   
        });
...
<div class="container">
  <h2>Enter Details</h2>
  <form action="getServiceId" id="MyWebServiceForm" method="post">
